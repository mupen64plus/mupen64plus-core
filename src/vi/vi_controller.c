...
int read_vi_regs(void* opaque, uint32_t address, uint32_t* value)
{
    ...
    if (reg == VI_CURRENT_REG)
    {
        ...
        if (g_alternate_vi_timing)
            ...
            vi->regs[VI_CURRENT_REG] = (vi->delay - (vi->next_vi - cp0_regs[CP0_COUNT_REG])) / DEFAULT_CPU_COUNT_PER_SCANLINE;
...
void vi_vertical_interrupt_event(struct vi_controller* vi)
{
    ...
    vi->delay = (vi->regs[VI_V_SYNC_REG] == 0)
            ? 500000
            : (vi->regs[VI_V_SYNC_REG] + 1) * DEFAULT_CPU_COUNT_PER_SCANLINE;

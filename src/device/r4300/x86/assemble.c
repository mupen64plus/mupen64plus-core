...
void init_assembler(struct r4300_core* r4300, void *block_jumps_table, int block_jumps_number, void *block_riprel_table, int block_riprel_number)
{
    ...
    {
        r4300->jumps_table = (struct jump_table *) block_jumps_table;
        r4300->jumps_number = block_jumps_number;
        r4300->max_jumps_number = r4300->jumps_number;
    }
    else
    {
        r4300->jumps_table = (struct jump_table *) malloc(1000*sizeof(struct jump_table));
        r4300->jumps_number = 0;
        r4300->max_jumps_number = 1000;
    }
}
...
void add_jump(struct r4300_core* r4300, unsigned int pc_addr, unsigned int mi_addr)
{
    ...
    {
        r4300->max_jumps_number += 1000;
        r4300->jumps_table = (struct jump_table *) realloc(r4300->jumps_table, r4300->max_jumps_number*sizeof(struct jump_table));
    }
...
void passe2(struct r4300_core* r4300, struct precomp_instr *dest, int start, int end, struct precomp_block *block)
{
    ...
    {
        r4300->recomp.code_length = r4300->jumps_table[i].pc_addr;
        if (dest[(r4300->jumps_table[i].mi_addr - dest[0].addr)/4].reg_cache_infos.need_map)
        {
            addr_dest = (unsigned int)dest[(r4300->jumps_table[i].mi_addr - dest[0].addr)/4].reg_cache_infos.jump_wrapper;
            put32(addr_dest-((unsigned int)block->code+r4300->recomp.code_length)-4);
        }
        else
        {
            addr_dest = dest[(r4300->jumps_table[i].mi_addr - dest[0].addr)/4].local_addr;
...
void jump_end_rel8(struct r4300_core* r4300)
{
    ...
    {
        DebugMessage(M64MSG_ERROR, "8-bit relative jump too long! From %x to %x", r4300->jump_start8, jump_end);

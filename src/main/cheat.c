...
int cheat_add_hacks(void)
{
    ...
    if (strncmp((char *)ROM_HEADER.Name, "THE LEGEND OF ZELDA", 19) == 0) {
        ...
        if (subscreen_address) {
            m64p_cheat_code subscreen_hack[] = {
                {subscreen_address, 0x0002},
            };
            cheat_add_new("HACK0", subscreen_hack, ARRAY_SIZE(subscreen_hack));

            if (credits_address[0]) {
                m64p_cheat_code credits_hack[] = {
                    {credits_address[0], 0x0320},
                    {credits_address[1], 0x0000},
                    {credits_address[2], 0xF809},
                    {credits_address[3], 0x0000},
                };
                cheat_add_new("HACK1", credits_hack, ARRAY_SIZE(credits_hack));
            }
        }
    }
